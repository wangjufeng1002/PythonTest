...
def process(page,limit):
    ...
    while pageIndex <= endIndex:
        ...
        #page1 = urllib.request.Request(rootUrl, headers=headers)
        ...
        clan_rows = re.findall(r'<li>(.*?)</li>',htmlCode, re.S | re.M | re.I)  # 清洗 <> 开始
        datas = []
        for row in clan_rows:
            try:
                if 'movie' in row:
                    date = re.findall(r'<span class=\'date\'>(.*?)</span>', row, re.M | re.I)
                    if date is None:
                        continue
                    uri = re.findall(r'href=\'/html/movie/pc/guochan_\d*\.html\'', row)
                    ...
                    data = Data('2023-'+date[0].replace('<span class=\'date\'>','').replace('</span>',''), dec, url)
                    datas.append(data)
            except:
