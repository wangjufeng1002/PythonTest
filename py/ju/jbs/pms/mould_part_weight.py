...
mould_update_template_sql = \
    '''update mes_pms.mould set part_weight = {part_weight} ,drain_weight = {drain_weight} where mould_code = '{mould_code}' ; '''

file_path = 'D:\\项目相关\\mould\\mould_part_weight'
    ...
    try:
        ...
        for rowIndex in range(3, sheet.max_row + 1):
            ...
            if ((data['模具编码'] is None or data['模具编码'] == '' or data['模具编码'] == 'NONE')
                    and (data['零件克重'] is None or data['零件克重'] == '' or data['零件克重'] == 'NONE')
                    and (data['水口克重'] is None or data['水口克重'] == '' or data['水口克重'] == 'NONE')):
                break

            if ((data['模具编码'] is not None and data['模具编码'] != '' and data['模具编码'] != 'NONE')
                    and (data['零件克重'] is not None and data['零件克重'] != '' and data['零件克重'] != 'NONE')
                    and (data['水口克重'] is not None and data['水口克重'] != '' and data['水口克重'] != 'NONE')):
                try:
                    data['模具编码'] = str(data['模具编码']).replace('\n', '').strip().upper()
                    data['零件克重'] = Decimal(data['零件克重']).quantize(Decimal("0.00"), rounding=ROUND_HALF_UP)
                    data['水口克重'] = Decimal(data['水口克重']).quantize(Decimal("0.00"), rounding=ROUND_HALF_UP)
                    shared_list.append(data)
                except:
                    err = "异常数据：{sheetName},行数：{row_index},模具编码：{mould_code}, 零件克重:{part_weight},水口克重:{drain_weight},".format(
                        mould_code=data['模具编码'], part_weight=data['零件克重'], drain_weight=data['水口克重'],
                        row_index=rowIndex, sheetName=sheetName)
                    #print(err)
...
def generate_mould_update_sql(results, sheet_name):
    ...
    for data in results:
        update_sql = mould_update_template_sql.format(mould_code=data['模具编码'],
                                                      part_weight=data['零件克重'], drain_weight=data['水口克重'])
...
if __name__ == '__main__':
    read_excel("D:\\项目相关\\mould\\mould_part_weight\\水口和产品克重2.xlsx", False, "生产一部")
    read_excel("D:\\项目相关\\mould\\mould_part_weight\\水口和产品克重2.xlsx", False, "生产二部")
    read_excel("D:\\项目相关\\mould\\mould_part_weight\\水口和产品克重2.xlsx", False, "生产三部")
    read_excel("D:\\项目相关\\mould\\mould_part_weight\\水口和产品克重2.xlsx", False, "生产四部")
