...
sql = "update wms_work.stock set wwg_idx = '{new_wwg_idx}',goods_code='{new_goods_code}' where wwg_idx = '{old_wwg_idx}' and goods_code='{old_goods_code}';"
...
def generate_update_sql():
    ...
    rows = sheets_.nrows
    count_1 = 0
    file = open("C:\\Users\\PC\Desktop\\替换\\wms库存.sql", "w", encoding='utf-8')
    insert_sql = []
    old_goods_codes = []
    goods_codes_map = {}
...
def read_file_print_sql():
    ...
    for index in range(1, rows):
        values = sheets_.row_values(rowx=index)
        old_goods_codes.append(values[0])
        new_goods_codes.append(values[3])
        goods_codes_map[values[0]] = values[3]

    goods_code_param = ','.join(repr(str(goods_code)) for goods_code in old_goods_codes)
    sql = "SELECT * FROM wms_work.`stock` where goods_code in (%s)" % (goods_code_param)
    print(sql)
...
if __name__ == '__main__':
    ...
    generate_update_sql()
    #read_file_print_sql()
    #print(get_ww_idx('WH0001'+'01K-GW04-3'+'001') == '7c13db82433a5152f2de256b15dd056b')
